...
-- Confetti Defaults.
local shapes = { UI:WaitForChild("CircularConfetti"), UI:WaitForChild("SquareConfetti") }
local colors = {
...
function _Confetti:Update()
	...
	if imageLabel then
		...
		self.OutOfBounds = (imageLabel.AbsolutePosition.X > ViewportSize.X and gravity.X > 0)
			or (imageLabel.AbsolutePosition.Y > ViewportSize.Y and gravity.Y > 0)
			or (imageLabel.AbsolutePosition.X < 0 and gravity.X < 0)
			or (imageLabel.AbsolutePosition.Y < 0 and gravity.Y < 0)
		...
		if newPower.Y < 0 then
			...
			imageLabel.Size = UDim2.fromOffset(self.DefaultSize, self.Size)
		end
	end
end
